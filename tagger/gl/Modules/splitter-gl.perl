...
my $lex = $abs_path."/lexicon/split_stored" ;

my $arrayref = retrieve($lex); 
my $NonVerb = $arrayref->[0];
my $Verb = $arrayref->[1];
...
my $acc = "o|os|a|as";
my $procl = "mas|mos|ma|mo|chos|chas|cho|cha|llo|lla|llos|llas|llelo|llela|llelos|llelas|nolo|nolos|nola|nolas|volo|volos|vola|volas";
###########################################################
...
##distribuíno - argüíunas
  ...
  my ($raiz, $sufixo) = $token =~ /\b([^\W]+[uü]íu)n([oa](s)?)\b/;
  my $raiz_lower = lowercase ($raiz);
...
##coméstelo 
  ...
  my ($raiz, $sufixo) = $token =~ /\b([^\W]+(?:te|che))l([oa](s)?)\b/;

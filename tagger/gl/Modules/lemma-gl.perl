...
sub lemma_pt {
  ...
  my $Det = "(o|a|os|as|um|uma|uns|umas|algum|alguns|alguma|algumas|todo|todos|toda|todas|vários|váriass|moito|moitos|moita|moitas)" ; ##determinantes par ver o contexto das ambiguas: desse, pelo...
  my $PrepCompl = "(de|con|para|sen)" ;  ##preposiçoes complemento de nome
     ...
     ## if   ( $tokens[$i] =~ /^$UpperCase$LowerCase+/ && ($StopWords->{$lowercase} && ($tokens[$k]  =~ /^(\#SENT\#|\<blank\>|\"|\“|\«|\.|\-|\s|\¿|\¡)$/) || ($i==0)) ) {   }##se em principio de frase a palavra maiuscula e uma stopword, nao fazemos nada
	  ...
	  if ($tokens[$i] =~ /[^\s]_[^\s]/ ) { 
		...
		$Tag{$tokens[$i]} =  $Entry->{$tokens[$i]};
         ...
         elsif ($tokens[$i] =~ /^pelo$/i && $tokens[$i+1] !~ /^$Det|$PrepCompl$/i) {
           $saida  = "por por SPS00";  
           push (@saida, $saida);
           $saida  = "o o DA0MS0";  
           push (@saida, $saida);
	 }
         elsif ($tokens[$i] =~ /^polos$/i && $tokens[$i+1] !~ /^$Det|$PrepCompl$/i) {
           $saida  = "por por SPS00";  
           push (@saida, $saida);
           $saida  = "o os DA0MS0";  
           push (@saida, $saida);

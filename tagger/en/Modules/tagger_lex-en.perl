...
foreach  my $line (@train) {  ##leitura treino
   ...
   if ($line =~ /<cat>/) { 
     ...
     (my $tmp) = $line =~ /<lex>([^<]*)</ ;
     #print STDERR "ProbCat: ---> #$tmp# \n";
     my  ($lex, $cat, $prob) = split (" ", $tmp);
     $ProbLex{$lex}{$cat}=$prob if ($prob);
     #print STDERR "CAT: ---> #$cat# \n";
  ...
  if ($entry[2] !~ /^$Border$/) {
    ...
    $pos++;
	 ...
	 if ($noamb[$pos]) {
        ...
        $tag = classif ($pos, \@Feat, \@Cat,  $Token[$pos]);
 ...
 foreach $cat (@C) {
  ...
  foreach $feat_restr (@F) {
       ...
       if (!$featFreq{$feat}) { 
        ...
        $ProbLex{$lex}{$cat}  = $smooth if ($ProbLex{$lex}{$cat}  ==0) ;
        ...
        $PostProb{$cat} = $PostProb{$cat} * $PriorProb{$cat}{$feat} * $ProbLex{$lex}{$cat};
